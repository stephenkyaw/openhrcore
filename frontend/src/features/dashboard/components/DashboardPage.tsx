...
function BarRow({ label, value, max, sub, tone = "accent" }: BarRowProps) {
...
export function DashboardPage({ onNav, onAskAgent }: any) {
  ...
  const upcomingHolidays = holidays
    .filter((h) => new Date(h.date).getTime() >= new Date(`${TODAY}T00:00:00`).getTime())
    .slice(0, 4);
  ...
  const activeLeaveToday = approvedLeave.filter(
    (r) =>
      new Date(r.from).getTime() <= new Date(`${TODAY}T00:00:00`).getTime() &&
      new Date(r.to).getTime() >= new Date(`${TODAY}T00:00:00`).getTime(),
  );
  const dashboardRows = [
  ...
  const deptRows = Object.entries(
    ...
    .map(([dept, value]) => ({ label: deptName(dept), value: Number(value) }))
                      ...
                      <TR key={r.id}>
                        ...
                        <TD className="text-[12px] text-muted-fg">
                          {Math.max(
                            0,
                            Math.round(
                              (todayTime - dateTime(r.submitted.slice(0, 10))) / MS_PER_DAY,
                            ),
                          )}
                          d
                        </TD>{" "}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[11px] text-muted-fg">
                        {h.country} ·{" "}
                        {Math.max(0, Math.round((dt.getTime() - todayTime) / MS_PER_DAY))}d away
                      </div>{" "}
                    </div>{" "}
                  </div>
                );
