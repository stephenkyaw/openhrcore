...
export function AgentWorkspace({ params, onNav }) {
  const { decideLeave, requests, balances, currentUser, employees } =
    useStore();
  ...
  function createSession() {
    const id = `s-${Date.now()}`;
    setSessions((cur) => [
      { id, title: "New Agent session", meta: "Now", count: 0 },
      ...cur,
    ]);
    setActiveSession(id);
    setMessages([]);
  ...
  return (
    <PageShell
      ...
      stats={[
        ...
        { label: "people", value: employees.length },
        { label: "approvals", value: pendingLeave },
        { label: "sessions", value: sessions.length },
        ]}
    >
      <div className="px-7 py-6 grid grid-cols-12 gap-4">
        <div className="col-span-12 xl:col-span-8">
          <Card className="overflow-hidden h-full flex flex-col min-h-[620px]">
            <CardHeader className="flex-none">
              <div>
                <CardTitle>Conversation</CardTitle>
                <Caption className="mt-0.5">
                  Audited HR assistant for questions, summaries, and module hand-offs.
                </Caption>
              </div>
            ...
            <div
              ref={scrollRef}
              className="border-t border-border-soft flex-1 min-h-[380px] overflow-y-auto scroll-thin px-5 py-5 bg-surface"
            >
              {!hasMessages && (
                <div className="mx-auto max-w-3xl">
                  <div className="rounded-lg border border-border-soft bg-card px-4 py-4">
                    <div className="flex items-start gap-3">
                      <div className="w-9 h-9 rounded-md border border-border-soft bg-accent-soft text-accent flex items-center justify-center flex-none">
                        <I.Sparkle size={16} />
                      </div>
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-[12.5px] text-muted-fg leading-relaxed">
                          Use a suggestion or type a precise HR question. The answer stays in this workspace and links back to the source module.
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="border-t border-border-soft bg-card p-3 flex-none">
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  handleSend();
                }}
                className="flex items-end gap-2 rounded-md border border-border-soft bg-card px-3 py-2 focus-within:ring-2 focus-within:ring-accent/25"
              >
                <I.Sparkle
                  size={14}
                  className="text-accent flex-none mb-1.5"
                />
                <textarea
                  value={input}
          ...
          <Card>
            <CardHeader>
              <div>
                ...
                <Caption className="mt-0.5">
                  Switch context or start a new Agent thread
                </Caption>
              </div>
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <Badge tone="outline" size="sm">
                        {active ? messages.length : session.count}
                      </Badge>
                    </div>
                  </button>
                );
          ...
          <Card>
            <CardHeader>
              <div>
                ...
                <Caption className="mt-0.5">
                  Common HR questions to start from
                </Caption>
              </div>
            </CardHeader>
            <CardBody className="space-y-1.5">
          ...
          <Card>
            <CardHeader>
              <div>
                ...
                <Caption className="mt-0.5">
                  Signals the Agent can check before you ask
                </Caption>
              </div>
            </CardHeader>
            <CardBody className="space-y-1.5">
