...
import { PROMPTS } from "./components/AgentWorkspaceData";

const todayDate = () => new Date(`${TODAY}T00:00:00`);

export function AgentIntro({ onPick }: any) {
  return (
    <div className="py-4">
      {" "}
...
function AgentResultView({ view, onNav, decideLeave }: any) {
              ...
              <div className="text-right">
                ...
                <div className="text-[10px] text-muted-fg">
                  ...
                  {Math.max(
                    0,
                    Math.round(
                      (new Date(`${e.probation_end}T00:00:00`).getTime() - todayDate().getTime()) /
                        86400000,
                    ),
                  )}
                  d left{" "}
...
export function AgentMessage({ m, onNav, decideLeave }: any) {
  if (m.role === "user") {
    return (
