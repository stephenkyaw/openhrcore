...
export function AgentPanel({ open, onClose, seed, onNav }: any) {
  ...
  useEffect(() => {
    if (open && seed && messages.length === 0)
      handleSend(
        seed,
  ...
  const push = (m: any) =>
    setMessages((mm) => [...mm, createAgentMessage(m)]);
  async function handleSend(text = "") {
    ...
    await runAgentTurn({
      ...
      push,
      delays: { thinking: 380, toolCall: 520, toolResult: 550, reply: 280 },
    });
    setBusy(false);
  }
  return (
    <Sheet open={open} onClose={onClose} width={520}>
      {" "}
      <div className="px-4 py-3 border-b border-border-soft bg-card flex items-center gap-2">
        {" "}
        ...
        <div className="flex-1">
          ...
          <div className="text-[11px] text-muted-fg">
            Agent parity with UI - {empName("e001")} - audited
          </div>{" "}
        </div>{" "}
        <Badge tone="outline">
          <I.Shield size={10} />
        ...
        <button
          onClick={onClose}
          className="p-1.5 rounded hover:bg-muted text-muted-fg ml-1"
        >
          <I.X size={14} />
        </button>{" "}
      </div>{" "}
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto scroll-thin px-4 py-4 space-y-3"
      >
        {" "}
      ...
      <div className="border-t border-border-soft p-3 bg-surface/70">
        {" "}
        <form
          onSubmit={(e) => {
            e.preventDefault();
          ...
          className="flex items-end gap-2 bg-card border border-border-soft rounded-md focus-within:ring-2 focus-within:ring-accent/25 px-2.5 py-1.5"
        >
          {" "}
