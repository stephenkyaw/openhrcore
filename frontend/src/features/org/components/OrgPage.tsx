...
function getReports(empId: string) {
  return EMPLOYEES.filter((e) => e.manager === empId);
}
function getDeptHead(deptId: string) {
  return (
    EMPLOYEES.find((e) => e.dept === deptId && getReports(e.id).length > 0) ||
    EMPLOYEES.find((e) => e.dept === deptId)
  );
}
function managerOptions({ exclude }: any = {}) {
  return (
    <>
      {" "}
      ...
      {EMPLOYEES.filter((e) => e.id !== exclude).map((e) => (
        <option key={e.id} value={e.id}>
          {e.first} {e.last} · {positionName(e.position)}
        </option>
      ))}{" "}
    </>
  );
...
function orgDefaults(action: any) {
...
function OrgDetailDialog({ detail, onClose, onEdit, onDelete, onNav }: any) {
      ...
      <div className="p-5 grid grid-cols-2 gap-4">
        ...
        {type === "department" && (
          <>
            ...
            <DetailRow label="Actual employees">
              {EMPLOYEES.filter((e) => e.dept === item.id).length}
            </DetailRow>{" "}
            <DetailRow label="Department head">
              {getDeptHead(item.id)
                ? `${getDeptHead(item.id).first} ${getDeptHead(item.id).last}`
                : "Unassigned"}
            </DetailRow>{" "}
          </>
        )}{" "}
        {type === "position" && (
          <>
            ...
            <DetailRow label="Department">{deptName(item.dept)}</DetailRow>{" "}
            <DetailRow label="Assigned employees">
              {EMPLOYEES.filter((e) => e.position === item.id).length}
            </DetailRow>{" "}
          </>
        )}{" "}
      </div>{" "}
        ...
        <div className="flex items-center gap-2">
          ...
          {type === "employee" && (
            <Button
              variant="outline"
              size="md"
              onClick={() => onNav("employees", item.id)}
            >
              <I.Users size={13} />
              Profile
            </Button>
...
function OrgFormFields({ mode, form, update, editingId }: any) {
  return (
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Select
                ...
                className="mt-1.5"
              >
                {managerOptions({ exclude: editingId })}
              </Select>
            </div>{" "}
            <div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Select
                ...
                className="mt-1.5"
              >
                {managerOptions({ exclude: form.employee })}
              </Select>
            </div>{" "}
            <div>
          ...
          <div>
            ...
            <Textarea
              ...
              className="mt-1.5"
              placeholder="Reorg, transfer, manager change..."
            />
          </div>{" "}
        </>
      )}{" "}
...
function OrgEditDialog({ edit, onClose, onSave }: any) {
  const [form, setForm] = useState(orgDefaults(edit));
  useEffect(() => {
    if (edit) setForm(orgDefaults(edit));
  }, [edit]);
  if (!edit) return null;
  ...
  return (
    <Dialog open onClose={onClose} width={560}>
      ...
      <div className="p-4 border-t border-border-soft flex items-center justify-end gap-2">
        ...
        <Button size="md" onClick={() => onSave(edit.type, edit.item, form)}>
          <I.Check size={13} />
          Save changes
        </Button>{" "}
      </div>{" "}
...
function OrgActionDialog({ action, onClose, onSubmit }: any) {
  const [form, setForm] = useState(orgDefaults(action));
  useEffect(() => {
    if (action) setForm(orgDefaults(action));
  }, [action]);
    ...
    <Dialog open onClose={onClose} width={560}>
      ...
      <div className="p-5 border-b border-border-soft flex items-start justify-between">
        ...
        <div>
          ...
          <div className="text-[12px] text-muted-fg mt-1">
            {isDelete
              ? "Confirm removal from this org setup."
              : "Complete the action details. The change is audit logged."}
          </div>{" "}
        </div>{" "}
        <Button variant="ghost" size="icon-sm" onClick={onClose}>
          <I.X size={13} />
      ...
      {isDelete ? (
        <div className="p-5 text-[13px]">
          {" "}
          Delete{" "}
          <b>{action.item?.name || action.item?.title || action.item?.id}</b>?
          Existing employees will be reassigned to their current fallback where
          possible.{" "}
        </div>
      ) : (
...
function OrgCard({ emp, onView, onNav, compact }: any) {
  const directs = getReports(emp.id).length;
  return (
    <div
      className={cn(
        "bg-card border border-border-soft rounded-md px-3.5 py-2.5 hover:border-accent/40 hover:shadow-sm flex items-center gap-2.5 text-left",
        compact ? "w-[190px]" : "w-[230px]",
      )}
    >
      {" "}
      <button
        onClick={() => onView("employee", emp)}
        className="flex items-center gap-2.5 flex-1 min-w-0 text-left focus-ring rounded"
      >
        {" "}
        <Avatar
...
function OrgBranch({ emp, onView, onNav, compact }: any) {
...
function OrgChart({ onView, onNav, compact }: any) {
    ...
    <div className="px-7 py-6 space-y-4">
      ...
      <div className="hidden">
        ...
        <Card>
          <Stat
            label="Managers"
            value={EMPLOYEES.filter((e) => getReports(e.id).length > 0).length}
            sub="With direct reports"
        ...
        <Card>
          <Stat
            label="Departments"
            value={new Set(EMPLOYEES.map((e) => e.dept)).size}
            sub="Represented"
            icon={<I.Building size={14} />}
          />
        </Card>{" "}
      </div>{" "}
      <div className="rounded-lg bg-card border border-border-soft p-8 min-w-max overflow-auto">
        {" "}
        {roots.length === 0 ? (
          <Empty
            title="No root node"
            sub="Assign at least one employee without a manager."
          />
        ) : (
...
function Departments({ onView, onEdit, onDelete, onAction }: any) {
  return (
    <div className="px-7 py-6 space-y-3">
      ...
      <div className="flex items-center justify-end">
        ...
        <Button
          size="sm"
          variant="outline"
          onClick={() => onAction("department", null, "Add department")}
        >
          <I.Plus size={11} />
          Add department
        </Button>{" "}
              ...
              return (
                ...
                >
                  ...
                  <TD className="text-[12.5px] text-muted-fg">
                    {d.parent ? deptName(d.parent) : "Top level"}
                  </TD>{" "}
                  <TD>
                    {head ? (
...
function Positions({ onView, onEdit, onDelete, onAction }: any) {
  return (
    <div className="px-7 py-6 space-y-3">
      {" "}
      <div className="flex items-center justify-end">
        ...
        <Button
          size="sm"
          variant="outline"
          onClick={() => onAction("position", null, "Add position")}
        >
          <I.Plus size={11} />
          Add position
        </Button>{" "}
            ...
            {POSITIONS.map((p) => (
              ...
              >
                ...
                <TD className="text-right font-mono">
                  {EMPLOYEES.filter((e) => e.position === p.id).length}
                </TD>{" "}
                <TD className="text-right">
                  {" "}
                  <div className="inline-flex gap-1">
...
function Reporting({ onView, onAction }: any) {
  const roots = EMPLOYEES.filter((e) => !e.manager);
  const overloaded = EMPLOYEES.filter((e) => getReports(e.id).length >= 4);
  const noDeptPosition = POSITIONS.filter(
    (p) => EMPLOYEES.filter((e) => e.position === p.id).length === 0,
  );
  return (
        ...
        <CardHeader>
          ...
          <div>
            ...
            <Caption className="mt-0.5">
              Move employees between managers and departments with an audit
              note.
            </Caption>{" "}
          </div>{" "}
          <Button
            size="sm"
            variant="outline"
            onClick={() => onAction("move", null, "Move employee")}
          >
            <I.Refresh size={11} />
            Move employee
          </Button>{" "}
            ...
            {EMPLOYEES.map((e) => (
              ...
              >
                ...
                <TD className="text-[12.5px]">
                  {e.manager ? (
                    `${empById(e.manager).first} ${empById(e.manager).last}`
                  ) : (
                    <Badge tone="outline" size="sm">
                      Root
                    </Badge>
                ...
                <TD className="text-[12.5px]">
                  {LOCATIONS.find((l) => l.id === e.loc)?.name}
                </TD>{" "}
                <TD className="text-right">
                  <Button
                    variant="ghost"
      ...
      <div className="space-y-3">
        ...
        <Card>
          <CardBody className="pt-4">
            <Stat
              label="Top-level employees"
              value={roots.length}
              sub={roots.map((e) => e.first).join(", ")}
              icon={<I.Shield size={14} />}
            />
          </CardBody>
        </Card>{" "}
        <Card>
          {" "}
          <CardHeader>
            <CardTitle>Manager span</CardTitle>
          </CardHeader>{" "}
            ...
            {overloaded.length ? (
              overloaded.map((e) => (
                <button
                  key={e.id}
                  onClick={() => onView("employee", e)}
                  className="w-full text-left px-2.5 py-2 rounded border border-border-soft hover:bg-muted/50"
                >
                  {" "}
                  ...
                  <div className="text-[11px] text-muted-fg">
                    {getReports(e.id).length} direct reports
                  </div>{" "}
                </button>
              ))
            ) : (
              <Empty
                title="No overload"
                sub="No manager has 4+ direct reports."
              />
            )}{" "}
          </CardBody>{" "}
        </Card>{" "}
        <Card>
          {" "}
          <CardHeader>
            <CardTitle>Unfilled positions</CardTitle>
          </CardHeader>{" "}
          <CardBody className="space-y-1.5">
            ...
            {noDeptPosition.slice(0, 5).map((p) => (
              <button
                key={p.id}
                onClick={() => onView("position", p)}
                className="w-full text-left text-[12px] px-2 py-1.5 rounded hover:bg-muted/50"
              >
                {p.title} · {deptName(p.dept)}
              </button>
            ))}{" "}
          </CardBody>{" "}
        </Card>{" "}
...
export function OrgPage({ onNav }: any) {
  ...
  const emptyDepartments = DEPARTMENTS.filter((d) =>
    EMPLOYEES.every((e) => e.dept !== d.id),
  ).length;
    ...
    if (act.kind === "delete") {
      ...
      if (list) {
        ...
        if (act.type === "department") {
          ...
          EMPLOYEES.forEach((e) => {
            if (e.dept === act.item.id) e.dept = fallback;
          });
          POSITIONS.forEach((p) => {
            if (p.dept === act.item.id) p.dept = fallback;
          });
        }
        if (act.type === "position") {
          const fallback = POSITIONS[0]?.id;
          EMPLOYEES.forEach((e) => {
            if (e.position === act.item.id) e.position = fallback;
          });
        }
      }
      logAudit({
        action: `org.${act.type}.delete`,
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-bg">
      ...
      <PageHero
        ...
        actions={
          <>
            ...
            <Button
              ...
              size="md"
              onClick={() =>
                actionItem("export", { id: tab }, "Export org data")
              }
            >
              <I.Download size={13} />
              Export
            ...
            <Button
              ...
              size="md"
              onClick={() =>
                actionItem("layout", { id: layout }, "Layout settings")
              }
            >
              <I.Sliders size={13} />
              Layout
            ...
            <Button
              size="md"
              onClick={() => actionItem("move", null, "Move employee")}
            >
              <I.Refresh size={13} />
              Move employee
            </Button>{" "}
        ...
        metrics={[
          ...
          { label: "Managers", value: managers, sub: "With reports" },
          {
            label: "Org risks",
            value: riskCount,
            sub: `${deptCount} departments represented`,
          },
        ]}
      />{" "}
      ...
      <div className="flex-1 overflow-y-auto scroll-thin">
        ...
        {tab === "reporting" && (
          <Reporting onView={viewItem} onAction={actionItem} />
        )}{" "}
      </div>{" "}
      <OrgDetailDialog
        detail={detail}
