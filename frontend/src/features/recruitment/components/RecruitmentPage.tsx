...
const PIPELINE_CONFIG = [
  {
    ...
    dept: "d1",
    stages: ["applied", "screen", "interview", "onsite", "offer", "hired"],
    sla: 7,
    status: "active",
  },
  {
...
const JOB_POSTINGS = [
  {
    ...
    channel: "LinkedIn",
    status: "published",
    url: "https://linkedin.com/jobs/mercury-backend",
    budget: 12000,
    applicants: 14,
  ...
  },
  {
    ...
    channel: "JobDB",
    status: "published",
    url: "https://jobdb.com/mercury-backend",
    budget: 8000,
    applicants: 6,
...
const INTEGRATIONS = [
  {
    ...
    sync: "two-way",
    token: "••••-lnkd-2026",
    scopes: "jobs,candidates,messages",
    lastSync: "2026-05-20T07:45:00Z",
  },
  {
    id: "int-jobdb",
    name: "JobDB Thailand",
    ...
    sync: "jobs-inbound",
    token: "••••-jobdb-88",
    scopes: "jobs,applicants",
    lastSync: "2026-05-19T11:15:00Z",
  },
  {
    id: "int-indeed",
    name: "Indeed",
    ...
    token: "",
    scopes: "jobs",
    lastSync: "2026-05-13T10:00:00Z",
  },
];
const CAREER_PAGES = [
  {
    id: "cp-main",
    name: "Mercury Careers",
    slug: "careers",
    status: "published",
    theme: "blue",
    locale: "en",
    jobs: ["j1", "j2", "j3", "j4", "j5"],
    seoTitle: "Careers at Mercury Labs",
  },
  {
    id: "cp-eng",
    ...
    locale: "en",
    jobs: ["j1", "j4", "j6"],
    seoTitle: "Engineering jobs at Mercury Labs",
  },
];
const SOURCE_CHANNELS = [
  {
    id: "src-careers",
    name: "Inbound · Careers page",
    applied: 18,
    advanced: 6,
    hired: 1,
    cpa: 0,
    status: "active",
    sub: "Organic applications from published career pages",
  },
  {
    id: "src-referral",
    name: "Referral",
    ...
    cpa: 2500,
    status: "active",
    sub: "Employee referrals with bonus policy",
  },
  {
    id: "src-linkedin",
    name: "LinkedIn",
    ...
    cpa: 380,
    status: "active",
    sub: "Synced from LinkedIn Recruiter",
  },
  {
    id: "src-outbound",
  ...
  },
  {
    ...
    cpa: 800,
    status: "active",
    sub: "Campus and community hiring events",
  },
];
const SCORECARD_TEMPLATE = [
  {
    id: "sc-dim-system",
    dim: "System design",
    desc: "Trade-offs, partitioning, data modeling, failure modes.",
    weight: 25,
  },
  {
    id: "sc-dim-code",
    dim: "Code craft",
    desc: "Idioms, readability, testing instincts, code review quality.",
    weight: 20,
  },
  {
    id: "sc-dim-product",
    dim: "Product sense",
    desc: "Can the candidate push back on features and explain why?",
    weight: 20,
  },
  {
    id: "sc-dim-ownership",
    dim: "Ownership",
    desc: "Evidence of end-to-end shipping with measurable impact.",
    weight: 20,
  },
  {
    id: "sc-dim-collab",
    dim: "Collaboration",
    desc: "Disagrees and commits, can mentor junior engineers.",
    weight: 15,
  },
];
...
function recDefaults(action) {
  ...
  return {
    ...
    stages: Array.isArray(item.stages)
      ? item.stages.join(", ")
      : item.stages || "applied, screen, interview, onsite, offer, hired",
    sla: item.sla ?? 7,
    ...
    <div className="p-5 space-y-3.5 max-h-[70vh] overflow-y-auto scroll-thin">
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Input
                value={form.locText}
                onChange={(e) => update("locText", e.target.value)}
                className="mt-1.5"
              />
            </div>{" "}
          </div>{" "}
      ...
      {mode === "source" && (
        <>
          {" "}
          <div>
            <Label>Source / channel name</Label>
            <Input
              value={form.name}
              onChange={(e) => update("name", e.target.value)}
              className="mt-1.5"
          ...
          <div className="grid grid-cols-4 gap-3">
            ...
            <div>
              ...
              <Input
                type="number"
                value={form.hired}
                onChange={(e) => update("hired", +e.target.value)}
                className="mt-1.5 font-mono"
              />
            </div>{" "}
            <div>
              ...
              <Input
                type="number"
                value={form.cpa}
                onChange={(e) => update("cpa", +e.target.value)}
                className="mt-1.5 font-mono"
              />
            </div>{" "}
          </div>{" "}
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Select
                value={form.cand || form.id}
                onChange={(e) => update("cand", e.target.value)}
                className="mt-1.5"
              >
                {CANDIDATES.map((c) => (
...
function RecActionDialog({ action, onClose, onSubmit }) {
  ...
  useEffect(() => {
    if (action) setForm(recDefaults(action));
  }, [action]);
...
function CandCard({ cand, onNav, onDragStart, onDragEnd, dragging }) {
  ...
  const avgRating = scs.length
    ? scs.reduce((s, x) => s + x.rating, 0) / scs.length
    : cand.rating || null;
  const daysInStage = Math.max(
    1,
    Math.round((todayTime - dateTime(cand.applied)) / MS_PER_DAY),
  );
  ...
  return (
    <div
      ...
      onDragEnd={onDragEnd}
      className={cn(
        "group w-full text-left bg-card border border-border-soft rounded-md p-2.5 hover:border-accent/50 hover:bg-muted/60 focus-ring cursor-grab active:cursor-grabbing transition-all duration-150",
        dragging && "opacity-40 scale-[0.98] border-accent ",
      )}
    >
      {" "}
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {avgRating != null && (
              <span className="text-[10px] font-mono tabular-nums text-warn flex items-center gap-0.5">
                ★ {avgRating.toFixed(1)}
              </span>
            )}{" "}
            <span className="ml-auto text-muted-fg/40 group-hover:text-muted-fg transition-colors">
              <I.Menu size={11} />
            </span>{" "}
          </div>{" "}
          <div className="text-[10.5px] text-muted-fg truncate">
            {cand.current}
          </div>{" "}
          <div className="text-[10px] text-muted-fg font-mono mt-0.5">
            {cand.loc} · {cand.exp}
          </div>{" "}
        </div>{" "}
      </div>{" "}
      <div className="mt-2 pt-2 border-t border-border-soft">
        {" "}
        <div className="text-[10.5px] text-muted-fg truncate mb-1.5">
          {job?.title || "Open role"}
        </div>{" "}
        <div className="flex items-center gap-1.5 text-[10px] text-muted-fg">
          ...
          {scs.length > 0 && (
            <span className="inline-flex items-center gap-0.5">
              {" "}
              <I.Check size={9} />
              {scs.length} scorecard{scs.length !== 1 ? "s" : ""}{" "}
            </span>
          )}{" "}
...
function Pipeline({ onNav, jobFilterDefault, onAction, onMove }) {
  ...
  const visible = (
    activePipe
      ? activePipe.stages
          .map((id) => REC_STAGES.find((s) => s.id === id))
          .filter(Boolean)
      : REC_STAGES
  ).filter((s) => s.id !== "rejected");
  const pipeJobIds = activePipe
    ? JOBS.filter((j) => j.dept === activePipe.dept).map((j) => j.id)
    : null;
  const cands = CANDIDATES.filter(
    (c) =>
      (jobFilter === "all" || c.job === jobFilter) &&
      (!pipeJobIds || pipeJobIds.includes(c.job)),
  );
  ...
  return (
    <div className="flex flex-col h-full">
      {" "}
      <div className="px-7 py-3 border-b border-border-soft flex items-center gap-2 bg-card flex-none">
        {" "}
        <Select
          value={jobFilter}
        ...
        <Button
          size="sm"
          variant="outline"
          onClick={() => onAction("stage", null, "Add pipeline stage")}
        >
          <I.Plus size={11} />
          Stage
        </Button>{" "}
        <Button
          size="sm"
          variant="outline"
          onClick={() => onAction("pipeline", null, "New pipeline")}
        >
          <I.Plus size={11} />
          Pipeline
        </Button>{" "}
        <div className="ml-auto flex items-center gap-3 text-[11.5px] text-muted-fg">
          ...
          <span>
            ...
            <b className="text-fg">Drag candidates</b> between stages to update
            the pipeline{" "}
          </span>{" "}
        </div>{" "}
      </div>{" "}
      <div className="flex-1 overflow-x-auto overflow-y-hidden scroll-thin bg-bg">
        {" "}
              ...
              <div key={stage.id} className="w-[278px] flex flex-col flex-none">
                ...
                <div className="flex items-center justify-between mb-2 px-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      ...
                      style={{ background: `oklch(0.65 0.13 ${stage.color})` }}
                    />{" "}
                    <span className="text-[12px] font-semibold">
                      {stage.label}
                    </span>{" "}
                    {dragCand && dragCand.stage !== stage.id && activeDrop && (
                  ...
                  <span className="inline-flex items-center gap-1">
                    {" "}
                    <button
                      className="text-[11px] font-mono tabular-nums text-muted-fg hover:text-fg"
                      onClick={() => onAction("stage", stage, "Edit stage")}
                    >
                      {col.length}
                    </button>{" "}
                    <Button
                      ...
                      size="icon-sm"
                      onClick={() =>
                        onAction("delete", stage, "Delete stage", "stage")
                      }
                    >
                      <I.X size={10} />
                    </Button>{" "}
                ...
                <div className="h-1 rounded-full bg-muted overflow-hidden mb-2 mx-1">
                  ...
                  <div
                    ...
                    style={{
                      width: `${pct}%`,
                      background: `oklch(0.65 0.13 ${stage.color})`,
                    }}
                  />{" "}
                </div>{" "}
                <div
                  ...
                  className={cn(
                    ...
                    activeDrop
                      ? "bg-accent-soft/60 border-accent/50 ring-2 ring-accent/15"
                      : "bg-card border-border-soft shadow-soft-sm",
                  )}
                >
                  {" "}
                  {col.map((c) => (
                  ...
                  {col.length === 0 && (
                    <div
                      className={cn(
                        ...
                        activeDrop
                          ? "border-accent text-accent bg-card/70"
                          : "border-border-soft",
                      )}
                    >
                      {" "}
                      {activeDrop
                        ? "Release to move candidate"
                        : "No candidates"}{" "}
                    </div>
                  )}{" "}
                </div>{" "}
...
function JobsList({ onNav, onAction }) {
  ...
  const statusTabs = ["open", "draft", "on-hold", "closed", "all"];
  return (
    <div className="px-7 py-6 h-full overflow-hidden">
      {" "}
      <Card className="overflow-hidden h-full flex flex-col">
        ...
        <div className="border-b border-border-soft bg-card">
          ...
          <div className="px-4 py-3 flex items-center gap-3">
              ...
              {statusTabs.map((s) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => setStatus(s)}
                    className={cn(
                      "h-7 px-3 rounded text-[12.5px] font-medium capitalize focus-ring transition-all",
                      active
            ...
            <div className="ml-auto text-[12px] text-muted-fg">
            ...
            <Button
              size="sm"
              variant="outline"
              onClick={() => onAction("job", null, "New job")}
            >
              <I.Plus size={11} />
              New job
            </Button>{" "}
        ...
        <div className="flex-1 overflow-y-auto scroll-thin bg-card">
                ...
                return (
                  ...
                  >
                    ...
                    <TD>
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-[12px] text-muted-fg">
                          {locationName(j.loc)} · {j.type} · {j.headcount}{" "}
                          headcount
                        </div>
                      </div>{" "}
                    </TD>{" "}
                    <TD className="text-[12.5px]">{deptName(j.dept)}</TD>{" "}
                    <TD className="text-[12.5px] text-muted-fg">
                      {hm ? hm.first : "—"} · {rc ? rc.first : "—"}
                    </TD>{" "}
                    <TD>
                    ...
                    <TD className="text-right font-mono tabular-nums text-[12.5px]">
                      {inFlight.length}
                      <span className="text-muted-fg"> / {cands.length}</span>
                    </TD>{" "}
                    <TD className="text-right font-mono tabular-nums text-[12.5px]">
                      {offerStage}
                    </TD>{" "}
                    ...
                    <TD className="text-right">
                      ...
                      <div className="inline-flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          onClick={(e) => {
                            ...
                            onAction(
                              "post",
                              {
                                job: j.id,
                                pipeline:
                                  PIPELINE_CONFIG.find((p) => p.dept === j.dept)
                                    ?.id || PIPELINE_CONFIG[0].id,
                              },
                              "Create job post",
                            );
                          }}
                        ...
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            onAction("delete", j, "Delete job", "job");
                          }}
                        >
                          <I.X size={12} />
                        </Button>
          ...
          {filtered.length === 0 && (
            <Empty title="No jobs match" sub="Try another status filter." />
          )}{" "}
        </div>{" "}
      </Card>{" "}
    </div>
...
function CandidatesList({ onNav, onAction }) {
  ...
  const filtered = CANDIDATES.filter((c) => {
    ...
    if (
      q &&
      !`${c.first} ${c.last} ${c.current} ${c.loc}`
        .toLowerCase()
  ...
  return (
    <div className="h-full flex flex-col">
      {" "}
      <div className="px-7 py-3 border-b border-border-soft bg-card flex items-center gap-2 flex-none">
        {" "}
        <div className="relative w-72">
          ...
          <Input
            ...
            placeholder="Search candidate, current company…"
            className="pl-7"
          />{" "}
        </div>{" "}
        <Select
        ...
        <Button
          size="sm"
          variant="outline"
          onClick={() => onAction("candidate", null, "Add candidate")}
        >
          <I.Plus size={11} />
          Add candidate
        </Button>{" "}
        ...
        <Table>
          ...
          <tbody>
            ...
            {filtered.map((c) => {
              ...
              const stageObj = REC_STAGES.find((s) => s.id === c.stage) || {
                label: c.stage,
                color: 220,
              };
              return (
                <TR
                  ...
                  onClick={() => onNav("recruitment", null, { candId: c.id })}
                >
                  {" "}
                  <TD>
                    {" "}
                  ...
                  <TD>
                    ...
                    <span className="inline-flex items-center gap-1.5 text-[12.5px]">
                      ...
                      <span
                        ...
                        style={{
                          background: `oklch(0.65 0.13 ${stageObj.color})`,
                        }}
                      />{" "}
                      {stageObj.label}{" "}
                    </span>{" "}
                  ...
                  <TD className="text-right">
                    ...
                    <div className="inline-flex gap-1">
                      ...
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          onAction("candidate", c, "Edit candidate");
                        }}
                      >
                        <I.Edit size={12} />
                      </Button>{" "}
    ...
    <div className="px-7 py-6 space-y-4">
      ...
      <div className="flex items-center justify-end">
        ...
        <Button
          size="sm"
          variant="outline"
          onClick={() => onAction("interview", null, "Schedule interview")}
        >
          <I.Plus size={11} />
          Schedule interview
        </Button>{" "}
      ...
      {Object.entries(byDate).map(([date, ivs]) => {
        ...
        return (
          <Card key={date}>
            {" "}
            <div className="px-4 py-2.5 border-b border-border-soft flex items-center gap-3 bg-bg">
              {" "}
              <div className="text-[13px] font-semibold">
                {d.toLocaleString("en", { weekday: "long" })}
              </div>{" "}
              <div className="text-[12px] text-muted-fg font-mono">{date}</div>{" "}
              <div className="ml-auto text-[11.5px] text-muted-fg">
            ...
            <div>
              ...
              {ivs.map((iv) => {
                ...
                return (
                  <div
                    key={iv.id}
                    className="px-4 py-3 border-b border-border-soft last:border-0 flex items-center gap-4"
                  >
                    {" "}
                    <div className="w-16 text-center flex-none">
                      {" "}
                      <div className="text-[15px] font-mono font-semibold tabular-nums">
                        {iv.time}
                      </div>{" "}
                    ...
                    <div className="flex items-center gap-2 flex-none">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-[12.5px] flex items-center gap-1.5">
                          {" "}
                          <Avatar
                            name={`${interviewer.first} ${interviewer.last}`}
                            hue={interviewer.hue}
                            size={20}
                          />{" "}
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          onAction("interview", iv, "Edit interview")
                        }
                      >
                        Open
                      </Button>{" "}
    ...
    <div className="px-7 py-6 space-y-3">
      ...
      <div className="flex items-center justify-end">
        ...
        <Button
          size="sm"
          variant="outline"
          onClick={() => onAction("offer", null, "Draft offer")}
        >
          <I.Plus size={11} />
          Draft offer
        </Button>{" "}
            ...
            <Card key={o.id}>
              ...
              <div className="px-4 py-3 border-b border-border-soft flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11.5px] text-muted-fg">
                    {job.title} · starts {o.start_date}
                  </div>{" "}
                </div>{" "}
                {o.status === "sent" && (
                  <Badge tone="warn">
                    <I.Send size={10} />
                    Sent · expires {o.expires}
                  </Badge>
                )}{" "}
                {o.status === "pending-approval" && (
                  <Badge tone="outline">Pending approval</Badge>
                )}{" "}
                {o.status === "accepted" && (
              ...
              <CardBody className="space-y-1.5 text-[12.5px]">
                ...
                <div className="flex justify-between">
                  <span className="text-muted-fg">Basic</span>
                  <span className="font-mono tabular-nums">
                    ฿{o.basic.toLocaleString()}
                  </span>
                </div>{" "}
                <div className="flex justify-between">
                  <span className="text-muted-fg">Housing</span>
                  <span className="font-mono tabular-nums">
                    ฿{o.house.toLocaleString()}
                  </span>
                </div>{" "}
                <div className="flex justify-between">
                  <span className="text-muted-fg">Transport</span>
                  <span className="font-mono tabular-nums">
                    ฿{o.trans.toLocaleString()}
                  </span>
                </div>{" "}
                <div className="flex justify-between pt-1.5 mt-1 border-t border-border-soft font-semibold">
                  {" "}
                  <span>Total / mo</span>
                  <span className="font-mono tabular-nums">
              ...
              <div className="border-t border-border-soft px-4 py-2.5 flex items-center justify-end gap-1.5">
                {" "}
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => onAction("offer", o, "Preview offer")}
                >
                  <I.Eye size={11} />
                  Preview letter
                </Button>{" "}
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => onAction("offer", o, "Revise offer")}
                >
                  <I.Refresh size={11} />
                  Revise
                </Button>{" "}
                ...
                {o.status === "sent" && (
                  <Button
                    size="sm"
                    onClick={() => onAction("offer", o, "Resend offer")}
                  >
                    <I.Mail size={11} />
                    Resend
                  </Button>
                ...
                <Button
                  variant="ghost"
                  size="icon-sm"
                  onClick={() => onAction("delete", o, "Delete offer", "offer")}
                >
                  <I.X size={11} />
                </Button>{" "}
              </div>{" "}
      ...
      <Card className="col-span-2">
        ...
        <CardBody className="space-y-3 text-[13px] leading-relaxed text-fg/90">
          ...
          <p>
            ...
            generation of HR infrastructure. You will own a vertical slice
            end-to-end — from data model through to AI agent tooling — in an
            open-source codebase used by real companies.{" "}
          </p>{" "}
          <div>
            {" "}
            <div className="font-semibold mb-1.5 mt-2">
              What you will do
            </div>{" "}
            <ul className="space-y-1 text-[12.5px] text-fg/85 list-disc pl-5">
              {" "}
              <li>
                Design and build feature-first vertical slices: service,
                repository, HTTP routes, and agent tools.
              </li>{" "}
              <li>
                Treat the service layer as the single source of truth — both UI
                and agent must call it.
              </li>{" "}
              <li>
                Write durable code: every state change recorded in the audit
                log; permissions enforced once.
              </li>{" "}
              <li>
                Partner with the agent runtime team to expose new tools as
                features ship.
              </li>{" "}
            </ul>{" "}
          </div>{" "}
          <div>
            ...
            <ul className="space-y-1 text-[12.5px] text-fg/85 list-disc pl-5">
              {" "}
              <li>
                6+ years building backend systems with non-trivial domain
                complexity.
              </li>{" "}
              <li>
                Strong product instinct — you can argue against a feature
                request that doesn't earn its complexity.
              </li>{" "}
              <li>
                Experience with multi-tenant or compliance-sensitive software is
                a strong plus.
              </li>{" "}
            </ul>{" "}
          </div>{" "}
      ...
      <div className="space-y-3">
        ...
        <Card>
          ...
          <CardBody className="space-y-2 text-[13px]">
            {" "}
            <div className="flex justify-between">
              <span className="text-muted-fg">Band</span>
              <span className="font-mono">L4 · Senior</span>
            </div>{" "}
            <div className="flex justify-between">
              <span className="text-muted-fg">Base range</span>
              <span className="font-mono">฿110K–฿165K</span>
            </div>{" "}
            <div className="flex justify-between">
        ...
        <Card>
          {" "}
          <CardHeader>
            <CardTitle>Hiring panel</CardTitle>
          </CardHeader>{" "}
      ...
      <Card>
        ...
        <CardHeader>
          ...
          <Button
            ...
            variant="outline"
            onClick={() =>
              onAction("scorecard", null, "Add scorecard dimension")
            }
          >
            <I.Plus size={11} />
            Add dimension
        ...
        <CardBody className="space-y-3.5">
          ...
          {SCORECARD_TEMPLATE.map((row) => (
            <div
              key={row.id}
              className="flex items-center gap-4 pb-3 border-b border-border-soft last:border-0 last:pb-0"
            >
              {" "}
              ...
              <div className="text-[12.5px] text-muted-fg flex-1">
                {row.desc}
              </div>{" "}
              <div className="flex items-center gap-1 flex-none">
                ...
                {[1, 2, 3, 4, 5].map((n) => (
                  <div
                    key={n}
                    className={cn(
                      "w-6 h-6 rounded border border-border-soft flex items-center justify-center text-[11px] font-mono",
                      n === 4
              ...
              <div className="flex items-center gap-1 flex-none">
                ...
                <Button
                  ...
                  size="icon-sm"
                  onClick={() =>
                    onAction("scorecard", row, "Edit scorecard dimension")
                  }
                >
                  <I.Edit size={12} />
                </Button>{" "}
  ...
  return (
    <div className="px-7 py-6">
      ...
      <Card>
        ...
        <CardBody>
          {" "}
          <ol className="relative ml-2 space-y-3 text-[12.5px] border-l border-border-soft pl-5">
            {" "}
            {[
              {
                d: "2026-05-17",
                a: "Mei Suzuki advanced to offer",
                who: "e011",
              },
              {
                d: "2026-05-15",
                a: "Adam Mendez scheduled for technical interview",
                who: "e011",
              },
              {
                d: "2026-05-14",
                a: "Lena Müller advanced to recruiter screen",
                who: "e011",
              },
              {
                d: "2026-05-11",
                a: 'JD updated — added "agent runtime" responsibility',
                who: "e001",
              },
              {
                d: "2026-05-08",
                a: "Reema Aziz applied via LinkedIn",
                who: "system",
              },
              ...
              <li key={i} className="relative">
                ...
                <div className="text-[11px] text-muted-fg font-mono">
                  {x.d} · {x.who === "system" ? "system" : empName(x.who)}
                </div>{" "}
              </li>
            ))}{" "}
          </ol>{" "}
...
function JobDetail({ jobId, onNav, onAction, onMove }) {
  ...
  if (!j) return <div className="px-7 py-6">Job not found.</div>;
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      {" "}
      <div className="px-7 pt-5 pb-4 border-b border-border-soft">
        {" "}
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[13px] text-muted-fg">
              {" "}
              {deptName(j.dept)} · {locationName(j.loc)} · {j.type} ·{" "}
            ...
            <div className="flex items-center gap-4 mt-2.5 text-[12.5px] text-muted-fg">
              ...
              {hm && (
                <span className="inline-flex items-center gap-1.5">
                  {" "}
                  <Avatar
                    name={`${hm.first} ${hm.last}`}
                    hue={hm.hue}
                    size={18}
                  />
                  Hiring · {hm.first} {hm.last}{" "}
                </span>
              )}{" "}
              {rc && (
                <span className="inline-flex items-center gap-1.5">
                  {" "}
                  <Avatar
                    name={`${rc.first} ${rc.last}`}
                    hue={rc.hue}
                    size={18}
                  />
                  Recruiter · {rc.first} {rc.last}{" "}
                </span>
              )}{" "}
              <span className="font-mono">
                Opened {j.opened || "draft"} · Target close{" "}
                {j.target_close || "—"}
              </span>{" "}
            </div>{" "}
          </div>{" "}
          <div className="flex items-center gap-2 flex-none">
            ...
            <Button
              variant="outline"
              size="md"
              onClick={() => onAction("job", j, "Edit job description")}
            >
              <I.Edit size={13} />
              Edit JD
            </Button>{" "}
            <Button
              size="md"
              onClick={() =>
                onAction("candidate", { job: jobId }, "Add candidate")
              }
            >
              <I.Plus size={13} />
              Add candidate
                  ...
                  <div className="flex-1 flex flex-col items-center gap-1.5 px-1">
                      ...
                      className={cn(
                        ...
                        current
                          ...
                          : passed
                            ...
                            : "bg-muted text-muted-fg/60 border-border-soft",
                      )}
                    >
                      {" "}
                      {passed ? "✓" : i + 1}{" "}
                    ...
                    <div
                      className={cn(
                        ...
                        current ? "font-semibold" : "text-muted-fg",
                      )}
                    >
                      {stage.label}
                    </div>{" "}
                  </div>{" "}
                  {i < REC_STAGES.length - 2 && (
                    <div
                      className={cn(
                        "flex-1 h-px",
                        passed ? "bg-accent/40" : "bg-border",
                      )}
                    />
                  )}{" "}
                </Fragment>
      ...
      <Card>
        ...
        <CardBody className="space-y-2 text-[12.5px]">
          ...
          <Field
            ...
            value={`${Math.max(1, Math.round((todayTime - dateTime(cand.applied)) / MS_PER_DAY))} days`}
            mono
          />{" "}
          <Field
            ...
            value={
              cand.salary_ask
                ? `฿${cand.salary_ask.toLocaleString()} / mo`
                : "—"
            }
            mono
          />{" "}
          <Field
            label="Avg scorecard"
            value={avg ? `★ ${avg.toFixed(2)} (${scs.length})` : "—"}
          />{" "}
          <Field
            label="Interviews completed"
            value={`${ivs.length} scheduled`}
          />{" "}
        </CardBody>{" "}
      </Card>{" "}
        ...
        <CardBody className="space-y-3 text-[13px]">
          ...
          <div>
            ...
            <ul className="space-y-1.5 text-[12.5px] text-fg/85">
              {" "}
              <li>
                <b>{cand.current}</b> — {cand.exp} · Owned distributed payments
                infrastructure; reduced p99 latency 38% over 18 months.
              </li>{" "}
              <li>
                <b>Previous · Datafold</b> — 2019–2023 · Led data-diff product
                from prototype to $4M ARR. Hired and managed a team of 6
                engineers.
              </li>{" "}
              <li>
                <b>Earliest · Allegro</b> — 2016–2019 · Backend on auction
                services, mentored 4 junior engineers to mid-level.
              </li>{" "}
            </ul>{" "}
          </div>{" "}
          <div>
...
function CandScorecards({ scs }: any) {
  if (scs.length === 0)
    return (
      <Empty
        title="No scorecards yet"
        sub="Scorecards appear as the panel submits them."
      />
    );
  return (
    <div className="space-y-3">
        ...
        return (
          <Card key={s.id}>
            ...
            <div className="p-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-0.5 text-warn">
                    ...
                    {Array.from({ length: 5 }).map((_, i) => (
                      <span
                        key={i}
                        className={i < s.rating ? "" : "opacity-20"}
                      >
                        ★
                      </span>
                    ))}{" "}
              ...
              <div className="text-[13px] text-fg/90 italic pl-11 leading-relaxed">
                "{s.notes}"
              </div>{" "}
            </div>{" "}
          </Card>
      ...
      <Empty
        ...
        action={
          <Button
            size="md"
            onClick={() =>
              onAction("interview", { cand: cand.id }, "Schedule interview")
            }
          >
            <I.Plus size={13} />
            Schedule interview
            ...
            return (
              <TR key={iv.id}>
                ...
                <TD className="font-mono text-[12.5px] tabular-nums">
                  {" "}
                  {iv.time}{" "}
                  <span className="text-muted-fg">({iv.dur}m)</span>{" "}
                </TD>{" "}
                ...
                <TD className="text-right">
                  ...
                  <div className="inline-flex gap-1">
                    ...
                    <Button
                      ...
                      size="icon-sm"
                      onClick={() =>
                        onAction("delete", iv, "Delete interview", "interview")
                      }
                    >
                      <I.X size={12} />
                    </Button>{" "}
      ...
      <Empty
        ...
        action={
          <Button
            size="md"
            onClick={() =>
              onAction("offer", { cand: cand.id, job: job.id }, "Draft offer")
            }
          >
            <I.Plus size={13} />
            Draft offer
  ...
  return (
    <div className="grid grid-cols-3 gap-4">
      ...
      <Card className="col-span-2">
        ...
        <CardHeader>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Button
              variant="ghost"
              size="icon-sm"
              onClick={() => onAction("offer", offer, "Edit offer")}
            >
              <I.Edit size={12} />
            </Button>{" "}
            <Button
              variant="ghost"
              size="icon-sm"
              onClick={() => onAction("delete", offer, "Delete offer", "offer")}
            >
              <I.X size={12} />
            </Button>{" "}
          </div>{" "}
        ...
        <CardBody>
          {" "}
          <div className="p-6 border border-border-soft rounded bg-card font-serif text-[13px] leading-relaxed space-y-3">
            {" "}
            <div className="font-sans text-[11px] uppercase tracking-widest text-muted-fg">
              Mercury Labs Co., Ltd.
            </div>{" "}
            ...
            <p>
              ...
              of <b>{job.title}</b>, joining the {deptName(job.dept)} team. Your
              start date is <b>{offer.start_date}</b>, reporting to{" "}
              {empName(job.hiring_manager)}.{" "}
            </p>{" "}
            <p>
              {" "}
              Your monthly compensation will be{" "}
              <b>฿{offer.total.toLocaleString()}</b> gross, comprising basic
              salary, housing, and transport allowances as detailed below. You
              will be eligible for standard statutory benefits and 15 days of
              annual leave.{" "}
            </p>{" "}
            <p>
              {" "}
              This offer is contingent on standard background checks and is
              valid until {offer.expires}. Please indicate acceptance by
              replying to this email or signing the attached letter.{" "}
            </p>{" "}
            <div className="pt-3">
              Warm regards,
              <br />
              <b>Anya Sirichai</b> · Head of People
            </div>{" "}
          </div>{" "}
        </CardBody>{" "}
      </Card>{" "}
      <div className="space-y-3">
        ...
        <Card>
          ...
          <CardBody className="space-y-1.5 text-[12.5px]">
            ...
            <div className="flex justify-between">
              <span className="text-muted-fg">Basic</span>
              <span className="font-mono">฿{offer.basic.toLocaleString()}</span>
            </div>{" "}
            <div className="flex justify-between">
              <span className="text-muted-fg">Housing</span>
              <span className="font-mono">฿{offer.house.toLocaleString()}</span>
            </div>{" "}
            <div className="flex justify-between">
              <span className="text-muted-fg">Transport</span>
              <span className="font-mono">฿{offer.trans.toLocaleString()}</span>
            </div>{" "}
            <div className="flex justify-between pt-1.5 mt-1 border-t border-border-soft font-semibold">
              {" "}
              <span>Monthly</span>
        ...
        <Card>
          ...
          <CardBody className="space-y-1.5 text-[12.5px]">
            {" "}
            <div className="flex justify-between">
              <span className="text-muted-fg">Grant</span>
              <span className="font-mono">0.12%</span>
            </div>{" "}
            <div className="flex justify-between">
              <span className="text-muted-fg">Vesting</span>
              <span>4y · 1y cliff</span>
            </div>{" "}
          </CardBody>{" "}
        </Card>{" "}
      </div>{" "}
    ...
    <Card>
      ...
      <CardBody className="space-y-4">
        ...
        {[
          {
            from: "agent",
            who: "Kofi Mensah · Recruiter",
            d: "2026-05-17",
            txt: `Hi ${cand.first}, thank you for your time on the onsite — the panel was impressed and we are putting together an offer. Expect formal letter by end of week.`,
          },
          {
            from: "cand",
            who: `${cand.first} ${cand.last}`,
            d: "2026-05-15",
            txt: "Thanks for the onsite invite — looking forward to meeting the team on Tuesday. Quick question on the system design round: should I prepare for green-field or extend an existing service?",
          },
          {
            from: "agent",
            who: "Kofi Mensah · Recruiter",
            d: "2026-05-13",
            txt: `Hi ${cand.first}, we'd love to invite you to a final round onsite. Sending logistics in a separate email.`,
          },
        ].map((m, i) => (
          <div
            key={i}
            className={cn(
              "flex gap-3",
              m.from === "cand" && "flex-row-reverse",
            )}
          >
            {" "}
            <Avatar
              name={m.who}
              hue={m.from === "cand" ? cand.hue : 70}
              size={32}
            />{" "}
            <div
              className={cn(
...
function CandidateDetail({ candId, onNav, onAction }) {
  ...
  if (!cand) return <div className="px-7 py-6">Candidate not found.</div>;
  const job = JOBS.find((j) => j.id === cand.job);
  const ivs = INTERVIEWS.filter((iv) => iv.cand === cand.id);
  const scs = SCORECARDS.filter((s) => s.cand === cand.id);
  ...
  const nextStage =
    REC_STAGES.filter((s) => s.id !== "rejected")[
      Math.min(
        REC_STAGES.filter((s) => s.id !== "rejected").findIndex(
          (s) => s.id === cand.stage,
        ) + 1,
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2.5 flex-wrap mb-1">
              ...
              >
                ...
                <span
                  className="w-1.5 h-1.5 rounded-full"
                  style={{ background: `oklch(0.65 0.13 ${stageObj.color})` }}
                />{" "}
                {stageObj.label}{" "}
              </Badge>{" "}
              {cand.rating && (
                <Badge tone="warn">★ {cand.rating.toFixed(1)}</Badge>
              )}{" "}
            </div>{" "}
            <div className="text-[13px] text-muted-fg">
              {cand.current} · {cand.loc} · {cand.exp} experience
            </div>{" "}
            <div className="flex items-center gap-4 mt-2.5 text-[12.5px] text-muted-fg">
              {" "}
              <span className="inline-flex items-center gap-1.5">
                {" "}
                <I.Mail size={12} /> {cand.first.toLowerCase()}.
                {cand.last.toLowerCase()}@example.com{" "}
              </span>{" "}
              <span className="inline-flex items-center gap-1.5">
                <I.Tag size={12} /> {cand.source}
              </span>{" "}
              <span className="inline-flex items-center gap-1.5">
                <I.Calendar size={12} /> Applied {cand.applied}
              </span>{" "}
            </div>{" "}
          </div>{" "}
          <div className="flex items-center gap-2 flex-none">
            ...
            <Button
              ...
              size="md"
              onClick={() =>
                onAction("candidate", cand, "Review candidate profile")
              }
            >
              <I.Doc size={13} />
              Resume
      ...
      <div className="flex-1 overflow-y-auto scroll-thin px-7 py-6">
        {" "}
        {tab === "overview" && (
          <CandOverview cand={cand} ivs={ivs} scs={scs} />
        )}{" "}
        {tab === "scorecards" && <CandScorecards scs={scs} />}{" "}
        {tab === "interviews" && (
          <CandInterviews ivs={ivs} cand={cand} onAction={onAction} />
        )}{" "}
        {tab === "offer" && (
          <CandOffer offer={offer} cand={cand} job={job} onAction={onAction} />
        )}{" "}
...
const TALENT_POOL = [
  {
    ...
    first: "Ravi",
    last: "Sundaram",
    skills: ["Go", "Distributed systems", "Postgres"],
    location: "Singapore, SG",
    lastTouched: "2026-03-30",
    rating: 4,
    hue: 60,
    why: "Solid SWE, didn’t make it past final for backend — try again for L4 next year",
  },
  {
    id: "tp2",
    first: "Sophie",
    ...
    location: "Paris, FR",
    lastTouched: "2026-02-15",
    rating: 5,
    hue: 320,
    why: "Top-tier frontend — keep warm for senior FE opening",
  },
  {
    id: "tp3",
    first: "Karthik",
    last: "Iyer",
    skills: ["PM", "Fintech", "API products"],
    location: "Bangalore, IN",
    lastTouched: "2026-01-20",
    rating: 4,
    hue: 30,
    why: "PM finalist, declined offer due to relocation. Stay in touch",
  },
  {
    id: "tp4",
    first: "Mei-Ling",
    last: "Wu",
    skills: ["Design systems", "Figma", "Brand"],
    location: "Taipei, TW",
    lastTouched: "2025-12-08",
    rating: 5,
    hue: 280,
    why: "Outstanding portfolio. Open to remote roles only — wait for next remote slot",
  },
  {
    id: "tp5",
    first: "Hassan",
    last: "Yusuf",
    skills: ["Data eng", "dbt", "Airflow"],
    location: "Dubai, AE",
    lastTouched: "2025-11-19",
    rating: 3.5,
    hue: 150,
    why: "Strong technical, slightly under bar on system design. Try at L3",
  },
  {
    id: "tp6",
    first: "Elena",
    last: "Petrova",
    skills: ["QA", "Test automation", "Cypress"],
    location: "Belgrade, RS",
    lastTouched: "2025-10-04",
    rating: 4,
    hue: 200,
    why: "QA lead — keep on file for when we open quality eng role",
  },
];
    ...
    <div className="px-7 py-6 space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <Card>
          <Stat
            label="People warm"
            value={TALENT_POOL.length}
            sub="Past finalists worth re-engaging"
        ...
        <Card>
          <Stat
            label="Top tier (★ 5)"
            value={TALENT_POOL.filter((p) => p.rating >= 5).length}
            sub="Reach out first for new reqs"
            icon={<I.Sparkle size={14} />}
          />
        </Card>{" "}
        <Card>
          <Stat
            label="Re-engaged this Q"
            value="4"
            sub="Of which 2 moved into pipeline"
            icon={<I.Refresh size={14} />}
          />
        </Card>{" "}
        <Card>
          <Stat
            label="Avg time on file"
            value="124d"
            sub="Refresh after 180d"
      ...
      <Card>
        ...
        <CardHeader>
          ...
          <div>
            ...
            <Caption>
              People we wanted but couldn’t hire — kept warm for the right
              opening.
            </Caption>{" "}
          </div>{" "}
          <div className="flex items-center gap-1.5">
            ...
            <Button
              ...
              variant="outline"
              onClick={() =>
                onAction("talent", { id: "match" }, "Match talent to roles")
              }
            >
              <I.Sparkle size={11} className="text-accent" />
              Match to open roles
            </Button>{" "}
            <Button
              size="sm"
              variant="outline"
              onClick={() => onAction("talent", null, "Add talent")}
            >
              <I.Plus size={11} />
              Add
            </Button>{" "}
        ...
        <Table>
          ...
          <THead>
            ...
            <TR className="hover:bg-transparent">
              ...
              <TH>Person</TH>
              <TH>Skills</TH>
              <TH>Rating</TH> <TH>Last touched</TH>
          ...
          <tbody>
            ...
            {TALENT_POOL.map((p) => (
              <TR
                key={p.id}
                className="cursor-pointer"
                onClick={() => onAction("talent", p, "Edit talent")}
              >
                {" "}
                <TD>
                  {" "}
                ...
                <TD className="text-[12px] text-muted-fg max-w-[280px] truncate">
                  {p.why}
                </TD>{" "}
                <TD className="text-right">
                  {" "}
                  <div className="flex items-center gap-1 justify-end">
                    ...
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        onAction("talent", p, "Re-engage talent");
                      }}
                    >
                      <I.Mail size={12} />
                    </Button>{" "}
                    ...
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        onAction("delete", p, "Delete talent", "talent");
                      }}
                    >
                      <I.X size={12} />
                    </Button>{" "}
...
function Sources({ onAction }) {
  const total = SOURCE_CHANNELS.reduce(
    (s, x) => ({
      applied: s.applied + x.applied,
      advanced: s.advanced + x.advanced,
      hired: s.hired + x.hired,
    }),
    ...
    <div className="px-7 py-6 space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <Card>
          <Stat
            label="Advanced"
            value={total.advanced}
            sub={`${Math.round((total.advanced / total.applied) * 100)}% advance rate`}
        ...
        <Card>
          <Stat
            label="Quality of source"
            value="Referral"
            sub="Best advance rate · 75%"
            delta="+22%"
      ...
      <Card>
        ...
        <CardHeader>
          ...
          <Button
            size="sm"
            variant="outline"
            onClick={() => onAction("source", null, "Add source")}
          >
            <I.Plus size={11} />
            Add source
          </Button>{" "}
        ...
        <Table>
          ...
          <THead>
            ...
            <TR className="hover:bg-transparent">
              ...
              <TH className="text-right">Advance %</TH> <TH>Funnel</TH>
              <TH className="text-right">Cost per applicant</TH>
              <TH />{" "}
            </TR>{" "}
          </THead>{" "}
          <tbody>
            ...
            {SOURCE_CHANNELS.map((s) => {
              ...
              return (
                <TR
                  key={s.id}
                  className="cursor-pointer"
                  onClick={() => onAction("source", s, "Edit source")}
                >
                  {" "}
                  <TD>
                    {" "}
                    <div className="text-[13px] font-medium">{s.name}</div>{" "}
                    <div className="text-[11px] text-muted-fg truncate max-w-[260px]">
                      {s.sub}
                    </div>{" "}
                  </TD>{" "}
                  ...
                  <TD className="text-right">
                    ...
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        onAction("delete", s, "Delete source", "source");
                      }}
                    >
                      <I.X size={12} />
                    </Button>{" "}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Card>
          ...
          <CardHeader>
            ...
            <Button
              size="sm"
              variant="outline"
              onClick={() => onAction("source", null, "Add channel")}
            >
              <I.Plus size={11} />
              Add channel
            </Button>{" "}
          ...
          <div className="border-t border-border-soft">
            ...
            {SOURCE_CHANNELS.map((c) => (
              <div
                key={c.id}
                className="px-4 py-2.5 border-b border-border-soft last:border-0 flex items-center justify-between gap-2"
              >
                {" "}
                <div>
                  {" "}
                ...
                <div className="flex items-center gap-1">
                  ...
                  <Button
                    variant="ghost"
                    size="icon-sm"
                    onClick={() => onAction("source", c, "Edit channel")}
                  >
                    <I.Edit size={12} />
                  </Button>{" "}
                </div>{" "}
        ...
        <Card>
          ...
          <CardHeader>
            ...
            <Button
              ...
              onClick={() =>
                onAction(
                  "source",
                  SOURCE_CHANNELS.find((s) => s.id === "src-referral"),
          ...
          <CardBody className="space-y-2.5 text-[13px]">
            {" "}
            <Field
              label="Active referral bonus"
              value="฿20,000 on 90-day mark"
            />{" "}
            <Field label="Senior role bonus" value="฿50,000 on hire (L5+)" />{" "}
            <Field
              label="Referrals this quarter"
              value="12 submitted · 2 hired"
              mono
            />{" "}
            <Field
              label="Top referrer"
              value="Marcus Tan · 4 referrals YTD"
            />{" "}
          </CardBody>{" "}
        </Card>{" "}
      </div>{" "}
      ...
      <Card>
        ...
        <CardHeader>
          ...
          <div>
            ...
            <Caption>
              Each job can publish into a department-specific pipeline.
            </Caption>{" "}
          </div>{" "}
          <Button
            size="sm"
            ...
            >
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <div>
                  ...
                  <div className="text-[11.5px] text-muted-fg">
                    {deptName(p.dept)} · SLA {p.sla}d · {p.stages.length} stages
                  </div>{" "}
                </div>{" "}
                <div className="flex items-center gap-1">
                  {" "}
                  <Badge tone={p.status === "active" ? "ok" : "outline"}>
                    {p.status}
                  </Badge>{" "}
                  <Button
                    variant="ghost"
                    size="icon-sm"
                    onClick={() => onAction("pipeline", p, "Edit pipeline")}
                  >
                    <I.Edit size={12} />
                  </Button>{" "}
                  <Button
                    ...
                    size="icon-sm"
                    onClick={() =>
                      onAction("delete", p, "Delete pipeline", "pipeline")
                    }
                  >
                    <I.X size={12} />
                  </Button>{" "}
              ...
              <div className="flex flex-wrap gap-1 mt-2">
                ...
                {p.stages.map((id) => {
                  ...
                  return (
                    <Badge key={id} tone="outline" size="sm">
                      {s?.label || id}
                    </Badge>
                  );
                })}{" "}
      ...
      <Card>
        ...
        <CardHeader>
          ...
          <Button
            size="sm"
            variant="outline"
            onClick={() => onAction("stage", null, "Add stage")}
          >
            <I.Plus size={11} />
            Add stage
          </Button>{" "}
        ...
        <div className="border-t border-border-soft">
          ...
          {REC_STAGES.map((s) => (
            <div
              key={s.id}
              className="px-4 py-2.5 border-b border-border-soft last:border-0 flex items-center gap-2"
            >
              {" "}
              <span
              ...
              <Button
                variant="ghost"
                size="icon-sm"
                onClick={() => onAction("stage", s, "Edit stage")}
              >
                <I.Edit size={12} />
              </Button>{" "}
              <Button
                variant="ghost"
                size="icon-sm"
                onClick={() => onAction("delete", s, "Delete stage", "stage")}
              >
                <I.X size={12} />
              </Button>{" "}
            </div>
    ...
    <div className="px-7 py-6 space-y-3">
      ...
      <div className="flex items-center justify-end">
        ...
        <Button
          size="sm"
          variant="outline"
          onClick={() => onAction("post", null, "Create job post")}
        >
          <I.Plus size={11} />
          Create job post
        </Button>{" "}
            ...
            {JOB_POSTINGS.map((p) => {
              ...
              return (
                <TR
                  key={p.id}
                  className="cursor-pointer"
                  onClick={() => onAction("post", p, "Edit job post")}
                >
                  {" "}
                  <TD>
                    {" "}
                  ...
                  <TD className="text-right">
                    ...
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        onAction("delete", p, "Delete job post", "post");
                      }}
                    >
                      <I.X size={12} />
                    </Button>{" "}
    ...
    <div className="px-7 py-6 grid grid-cols-3 gap-3">
      ...
      <div className="col-span-3 flex justify-end">
        ...
        <Button
          size="sm"
          variant="outline"
          onClick={() => onAction("integration", null, "Add integration")}
        >
          <I.Plus size={11} />
          Add integration
        </Button>{" "}
      ...
      {INTEGRATIONS.map((i) => (
        <Card
          ...
          onClick={() => onAction("integration", i, "Edit integration")}
        >
          {" "}
          <CardHeader>
            {" "}
          ...
          <CardBody className="space-y-2 text-[12.5px]">
            ...
            <Field
              label="Last sync"
              value={i.lastSync?.replace("T", " ").slice(0, 16) || "Never"}
              mono
            />{" "}
            <div className="flex gap-1 pt-2">
              ...
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  onAction("integration", i, "Sync integration");
                }}
              >
                <I.Refresh size={11} />
                Sync
              ...
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  onAction("integration", i, "Edit integration");
                }}
              >
                <I.Edit size={11} />
                Edit
              ...
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  onAction("delete", i, "Delete integration", "integration");
                }}
              >
                <I.X size={11} />
              </Button>{" "}
    ...
    <div className="px-7 py-6 grid grid-cols-2 gap-3">
      ...
      <div className="col-span-2 flex justify-end">
        ...
        <Button
          size="sm"
          variant="outline"
          onClick={() => onAction("career", null, "New career page")}
        >
          <I.Plus size={11} />
          New career page
        </Button>{" "}
      ...
      {CAREER_PAGES.map((p) => (
        <Card
          key={p.id}
          className="cursor-pointer hover: transition-shadow"
          onClick={() => onAction("career", p, "Edit career page")}
        >
          {" "}
          <CardHeader>
            {" "}
            <div>
              ...
              <Caption className="mt-0.5">
                /{p.slug} · {p.locale} · {p.jobs.length} jobs
              </Caption>{" "}
            </div>{" "}
          ...
          <CardBody className="space-y-2">
            ...
            <div className="flex justify-end gap-1 pt-2">
              ...
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  onAction("career", p, "Preview career page");
                }}
              >
                <I.Eye size={11} />
                Preview
              ...
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  onAction("career", p, "Edit career page");
                }}
              >
                <I.Edit size={11} />
                Edit
              ...
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  onAction("delete", p, "Delete career page", "career");
                }}
              >
                <I.X size={11} />
              </Button>{" "}
...
export function RecruitmentPage({ params, onNav }) {
  ...
  const actionItem = (mode, item, title, deleteType = null) => {
    if (mode === "export") {
      logAudit({
        action: "recruitment.export",
        entity: `recruitment:${tab}`,
        meta: { tab },
      });
    ...
    if (mode === "talent" && item?.id === "match") {
      logAudit({
        action: "recruitment.talent.match",
        entity: "talent_pool:all",
        meta: { open_roles: JOBS.filter((j) => j.status === "open").length },
      });
  ...
  const submitAction = (act, form) => {
    ...
    if (act.mode === "delete") {
      ...
      if (type === "candidate") {
        for (let i = INTERVIEWS.length - 1; i >= 0; i--)
          if (INTERVIEWS[i].cand === id) INTERVIEWS.splice(i, 1);
        for (let i = OFFERS.length - 1; i >= 0; i--)
          if (OFFERS[i].cand === id) OFFERS.splice(i, 1);
      }
      if (type === "job") {
        for (let i = JOB_POSTINGS.length - 1; i >= 0; i--)
          if (JOB_POSTINGS[i].job === id) JOB_POSTINGS.splice(i, 1);
        CANDIDATES.filter((c) => c.job === id).forEach((c) => {
          c.stage = "rejected";
        });
      }
      if (type === "pipeline") {
        for (let i = JOB_POSTINGS.length - 1; i >= 0; i--)
          if (JOB_POSTINGS[i].pipeline === id) JOB_POSTINGS.splice(i, 1);
      }
      if (type === "stage") {
        PIPELINE_CONFIG.forEach((p) => {
          p.stages = p.stages.filter((s) => s !== id);
        });
        CANDIDATES.filter((c) => c.stage === id).forEach((c) => {
          c.stage = "applied";
        });
      }
      logAudit({
        action: `recruitment.${type}.delete`,
        entity: `${type}:${id}`,
        meta: {},
      });
      bump();
    ...
    if (type === "candidate") {
      saved = upsert(CANDIDATES, {
        ...
        salary_ask:
          form.salary_ask === "" ? null : Number(form.salary_ask) || null,
      });
    }
    if (type === "stage") {
    ...
    if (type === "career") {
      saved = upsert(CAREER_PAGES, {
        ...
        jobs:
          act.item?.jobs ||
          JOBS.filter((j) => j.status === "open").map((j) => j.id),
    ...
    if (type === "offer") {
      ...
      saved = upsert(OFFERS, {
        ...
        job:
          act.item?.job ||
          CANDIDATES.find((c) => c.id === form.cand)?.job ||
          JOBS[0]?.id,
  ...
  return (
      ...
      actions={
        <>
          ...
          <Button
            ...
            size="md"
            onClick={() =>
              actionItem("export", { id: tab }, "Export recruitment data")
            }
          >
            <I.Download size={13} />
            Export
          </Button>{" "}
          <Button size="md" onClick={() => actionItem("job", null, "New job")}>
            <I.Plus size={13} />
            New job
          </Button>{" "}
        </>
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto scroll-thin">
        {" "}
        {tab === "jobs" && (
          <JobsList onNav={onNav} onAction={actionItem} />
        )}{" "}
        {tab === "pipeline" && (
          <Pipeline
            onNav={onNav}
            jobFilterDefault={null}
            onAction={actionItem}
            onMove={moveCandidate}
          />
        )}{" "}
        ...
        {tab === "candidates" && (
          <CandidatesList onNav={onNav} onAction={actionItem} />
        )}{" "}
