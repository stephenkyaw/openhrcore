...
import { ACCENTS, ACCENTS_BY_HEX, HEX_BY_ACCENT, TWEAK_DEFAULTS } from '@/config/theme';
...
export function App() {
  ...
  const [route, setRoute] = useState({ view: 'agent', params: null });
  const [paletteOpen, setPaletteOpen] = useState(false);
  const [authed, setAuthed] = useState(true);

  const onNav = useCallback((view, id, params) => {
    setRoute({ view, params: id ? { id, ...(params || {}) } : params || null });
  }, []);

  const onAskAgent = useCallback((q) => {
  ...
  const densityClass =
    ...
    t.density === 'comfy'   ? 'density-comfy'   : '';

  if (!authed) {
    return (
      <Login onSignIn={() => { setAuthed(true); setRoute({ view: 'agent', params: null }); }} />
    );
  }

  return (
    <div className={cn('h-screen w-screen flex bg-bg text-fg overflow-hidden', densityClass)}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        />
        <main className="flex-1 overflow-hidden bg-bg border-l border-border-soft shadow-soft-sm">
