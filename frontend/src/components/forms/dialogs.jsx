...
export function NewEmployeeDialog({ open, onClose, onCreated }) {
  ...
  const probEnd =
    probMonths > 0
      ? fmt(
          new Date(
            new Date(hire).setMonth(new Date(hire).getMonth() + probMonths),
          ),
        )
      : null;
  ...
  function submit() {
    ...
    BASE_BALANCES[id] = {
      lt1: { granted: 15, used: 0, pending: 0 },
      lt2: { granted: 10, used: 0, pending: 0 },
    };
    logAudit({
      action: "employee.create",
      entity: `employee:${id}`,
      meta: { code, dept, position },
    });
    bump();
  ...
  return (
    <Dialog open={open} onClose={onClose} width={620}>
      ...
      <FormHeader
        ...
        title="Add employee"
        sub="Creates an employment record and a user account."
        onClose={onClose}
      />{" "}
      <div className="p-5 space-y-3.5 max-h-[70vh] overflow-y-auto scroll-thin">
        ...
        >
          ...
          <Input
            ...
            placeholder="hiro@mercury.co"
            type="email"
          />{" "}
        </FormField>{" "}
        <div className="pt-2 mt-2 border-t border-border-soft">
          {" "}
          <div className="text-[10.5px] uppercase tracking-wider text-muted-fg font-medium mb-2.5">
            Employment
          </div>{" "}
          <FormGrid>
            {" "}
            ...
            <FormField label="Position" required>
              ...
              >
                {" "}
                {POSITIONS.filter((p) => p.dept === dept || true).map((p) => (
            ...
            <FormField
              ...
              hint={probEnd ? `Ends ${probEnd}` : "No probation"}
            >
              {" "}
              <Select
                value={probMonths}
                onChange={(e) => setProbMonths(+e.target.value)}
              >
                {" "}
            ...
            <FormField label="Employee code" hint="Auto-generated">
              {" "}
              <Input value={code} readOnly className="font-mono bg-card" />{" "}
            </FormField>{" "}
          </FormGrid>{" "}
        </div>{" "}
        <div className="pt-2 mt-2 border-t border-border-soft flex items-center justify-between">
          {" "}
      ...
      >
        ...
        <Button size="md" onClick={submit} disabled={!valid}>
          <I.Plus size={13} />
          Add employee
        </Button>{" "}
      </FormFooter>{" "}
...
export function NewJobDialog({ open, onClose, onCreated }) {
  ...
  function submit() {
    ...
    const job = {
      ...
      priority,
      opened: publishStatus === "draft" ? null : fmt(TODAY),
      target_close: target || null,
      headcount,
      type,
    };
    JOBS.unshift(job);
    ...
    bump();
    toast(
      `Job ${publishStatus === "draft" ? "saved as draft" : "opened"}: ${title}`,
    );
    onCreated && onCreated(id);
    onClose();
  }
  return (
    <Dialog open={open} onClose={onClose} width={680}>
      {" "}
      <FormHeader
        eyebrow="Recruitment · New requisition"
        title="Open a new job"
        sub="Defines the role, panel, and budget before sourcing candidates."
        onClose={onClose}
      />{" "}
      <div className="p-5 space-y-3.5 max-h-[72vh] overflow-y-auto scroll-thin">
        ...
        <FormGrid>
          ...
          <FormField label="Recruiter">
            ...
            >
              {" "}
              {EMPLOYEES.filter((e) => e.dept === "d4").map((e) => (
                <option key={e.id} value={e.id}>
                  {e.first} {e.last}
                </option>
              ))}{" "}
        ...
        <FormGrid cols={3}>
          ...
          <FormField label="Type">
            ...
            <Select value={type} onChange={(e) => setType(e.target.value)}>
              ...
              <option>Contract · 6mo</option>
              <option>Contract · 12mo</option>
        ...
        <FormField label="Description" hint="Markdown supported">
          ...
          <Textarea
            ...
            placeholder="What will this person own? What does success look like in 6 months?"
          />{" "}
        </FormField>{" "}
      </div>{" "}
      <FormFooter
        hint={
          <>
            ...
            Agent can also draft the JD if you leave it blank
          </>
        }
      >
        {" "}
...
export function NewCandidateDialog({ open, onClose, jobId, onCreated }) {
  ...
  const [job, setJob] = useState(
    jobId || JOBS.find((j) => j.status === "open")?.id,
  );
  ...
  return (
    <Dialog open={open} onClose={onClose} width={620}>
      {" "}
      <FormHeader
        eyebrow="Recruitment · Add candidate"
        title="New candidate"
        sub="Adds them to the pipeline at the selected stage."
        onClose={onClose}
      />{" "}
      <div className="p-5 space-y-3.5 max-h-[70vh] overflow-y-auto scroll-thin">
        {" "}
        ...
        <FormGrid>
          {" "}
          <FormField label="Current role" className="col-span-2">
            <Input
              value={current}
        ...
        <FormGrid cols={3}>
          ...
          <FormField label="Source">
            ...
            <Select value={source} onChange={(e) => setSource(e.target.value)}>
              ...
              <option>Recruiter outbound</option>
        ...
        <FormField label="Notes" hint="Visible to the hiring panel">
          ...
          <Textarea
            ...
            placeholder="What stood out? Where did they come from?"
          />{" "}
        </FormField>{" "}
      </div>{" "}
      <FormFooter>
...
export function CheckInDialog({ open, onClose }) {
  ...
  function submit() {
    ...
    const idx = ATTENDANCE.findIndex(
      (a) => a.date === rec.date && a.emp === currentUser,
    );
    if (idx >= 0) ATTENDANCE.splice(idx, 1);
    ATTENDANCE.unshift(rec);
    logAudit({
      action: "attendance.checkin",
      entity: `attendance:${rec.id}`,
      meta: { time, source, wfh },
    });
    bump();
  ...
  return (
    <Dialog open={open} onClose={onClose} width={460}>
      ...
      <FormHeader
        ...
        title="Check in"
        sub="Records your arrival. Location + device captured for audit."
        onClose={onClose}
      />{" "}
      <div className="p-5 space-y-3.5">
        {" "}
        <div className="flex items-center gap-3 p-3 rounded-md border border-border-soft bg-card">
          {" "}
          ...
          <div className="flex-1">
            {" "}
            <div className="text-[11px] uppercase tracking-wider text-muted-fg font-medium">
              Current time
            </div>{" "}
          ...
          {isLate ? (
            <Badge tone="warn">
              <I.Clock size={10} />
              Late ·{" "}
              {Math.max(0, now.getHours() * 60 + now.getMinutes() - 9 * 60)}min
              after 09:00
            </Badge>
          ) : (
            <Badge tone="ok">
        ...
        {wfh && (
          <div className="text-[12px] text-muted-fg flex items-center gap-1.5 px-1">
            {" "}
            <I.Globe size={12} />
            Marked WFH — IP + agent recorded, no geofence required.{" "}
          </div>
        )}{" "}
      </div>{" "}
      <FormFooter
        hint={
          <>
            <I.Shield size={11} />
            Audited as <span className="font-mono">attendance.checkin</span>
          </>
        }
      >
        {" "}
  ...
  return (
    <Dialog open={open} onClose={onClose} width={520}>
      ...
      <FormHeader
        ...
        sub="Creates a draft. Recalculate before previewing or committing."
        onClose={onClose}
      />{" "}
      <div className="p-5 space-y-3.5">
        {" "}
      ...
      <FormFooter
        hint={
          <>
            ...
            Draft only — does not issue payslips
          </>
        }
      >
        {" "}
  ...
  return (
    <Dialog open={open} onClose={onClose} width={480}>
      ...
      <FormHeader
        ...
        sub="Departments form the org tree. Parent makes it a sub-department."
        onClose={onClose}
      />{" "}
      <div className="p-5 space-y-3.5">
        {" "}
      ...
      <FormFooter>
        ...
        <Button size="md" onClick={submit} disabled={!name.trim()}>
          <I.Plus size={13} />
          Create department
        </Button>{" "}
      </FormFooter>{" "}
  ...
  return (
    <Dialog open={open} onClose={onClose} width={480}>
      ...
      <FormHeader
        eyebrow="Company · Structure"
        title="New position"
        sub="Positions are job titles attached to a department."
        onClose={onClose}
      />{" "}
      <div className="p-5 space-y-3.5">
        {" "}
  ...
  return (
    <Dialog open={open} onClose={onClose} width={480}>
      ...
      <FormHeader
        eyebrow="Company · Structure"
        title="New location"
        sub="Used for time zones, holiday calendars, and (later) geofencing."
        onClose={onClose}
      />{" "}
      <div className="p-5 space-y-3.5">
        {" "}
  ...
  return (
    <Dialog open={open} onClose={onClose} width={460}>
      ...
      <FormHeader
        eyebrow="Company · Structure"
        title="Add holiday"
        sub="Public holidays are excluded from leave-day deduction automatically."
        onClose={onClose}
      />{" "}
      <div className="p-5 space-y-3.5">
        {" "}
        ...
        <FormField label="Name" required>
          <Input
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="e.g. Asarnha Bucha Day"
            autoFocus
          />
        </FormField>{" "}
...
export function NewRoleDialog({ open, onClose, onCreated }) {
  const { toast, logAudit, bump } = useStore();
  const [name, setName] = useState("");
  ...
  return (
    <Dialog open={open} onClose={onClose} width={560}>
      ...
      <FormHeader
        ...
        title="New role"
        sub="Create the role first, then update permissions from the role editor."
        onClose={onClose}
      />{" "}
      <div className="p-5 space-y-4">
        ...
        <div className="rounded-lg border border-border-soft bg-surface/55 px-3 py-2.5 text-[12.5px] text-muted-fg">
          {" "}
          This role will start with 0 permissions. After creation, it will be selected in
          Roles & permissions so you can turn access on by module.
        </div>{" "}
      </div>{" "}
      <FormFooter
        hint={
          <>
            <I.Shield size={11} />
            Role applies to 0 users until you assign it
          </>
        }
      >
        {" "}
    ...
    <Dialog open={open} onClose={onClose} width={520}>
      ...
      <div className="p-5 space-y-3.5">
        ...
        <FormGrid cols={3}>
          ...
          <FormField label="Carry forward (d)">
            <Input
              type="number"
              value={carry}
            ...
            {[165, 25, 280, 220, 320, 70, 0].map((h) => (
              <button
                ...
                className={cn(
                  ...
                  color === h ? "ring-fg" : "ring-transparent",
                )}
