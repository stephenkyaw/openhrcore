...
export function TweakSection({ label, children = null }: any) {
  return (
    <>
      {" "}
      <div className="twk-sect">{label}</div> {children}{" "}
    </>
  );
}
export function TweakRow({ label, value = null, children, inline = false }: any) {
  return (
        ...
        {options.map((o, i) => {
          ...
          return (
            ...
            >
              ...
              {on && <SwatchCheck light={isLight(hero)} />}{" "}
            </button>
          );
        })}{" "}
      </div>{" "}
