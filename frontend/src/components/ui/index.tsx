...
export function Textarea({ className = "", rows = 3, ...rest }: any) {
  return (
    <textarea
      rows={rows}
      className={cn(
...
export function Label({ children, className = "", ...rest }: any) {
  return (
    <label
      className={cn(
...
export function Dot({ tone = "ok", className = "", pulse = false }: any) {
  const tones = {
    ok: "bg-ok",
    warn: "bg-warn",
    danger: "bg-danger",
...
export function TierPill({ tier }: any) {
...
export function Avatar({ name, hue = 220, size = 28, className = "" }: any) {
  const initials = name
    .split(" ")
    .map((s) => s[0])
    .slice(0, 2)
...
function stripTableWhitespace(children: any): any {
  return Children.toArray(children)
    .filter((child) => !(typeof child === "string" && child.trim() === ""))
    .map((child) => {
      if (!isValidElement(child)) return child;
      if (["thead", "tbody", "tfoot", "tr"].includes(child.type as string)) {
...
export const TR = ({ children, className = "", ...rest }: any) => (
  <tr
    className={cn(
...
export const TD = ({ children, className = "", ...rest }: any) => (
  <td className={cn("px-4 py-3.5 align-middle", className)} {...rest}>
    {children}
  </td>
);

export function Tabs({ value, onChange, items, className = "" }: any) {
  return (
    <div
      className={cn(
...
export const Kbd = ({ children, className = "" }: any) => (
  <kbd
    className={cn(
...
export function Stat({ label, value, sub, delta, icon, className = "" }: any) {
...
export function InlineStats({ items = [], className = "" }: any) {
  return (
...
export function SectionHeader({ title, sub, actions, stats = [], className = "" }: any) {
  return (
 ...
 */
export function StatDeprecated({ label, value, sub, delta, icon, className = "" }: any) {
  const isNeg = delta && (delta.startsWith("-") || delta.startsWith("−"));
  return (
    <div className={cn("px-4 py-3 flex items-center justify-between gap-3 min-w-0", className)}>
      <div className="min-w-0">
...
export function Empty({ title, sub, action }: any) {
  return (
...
export function Dialog({ open, onClose, children, width = 520 }: any) {
  if (!open) return null;
  return (
...
export const Caption = ({ children, className = "" }: any) => (
...
export function PageKicker({ children, className = "" }: any) {
  return (
    <div
      className={cn(
...
export function PageHeader({ title, sub, actions, eyebrow }: any) {
  return (
...
export function FilterChip({ children, onClear }: any) {
  return (
...
export function SectionTitle({ children, sub, action, className = "" }: any) {
  return (
...
export function leaveStatusBadge(status: any) {
  if (status === "approved")
    return (
      <Badge tone="ok">
        <I.Check size={10} />
